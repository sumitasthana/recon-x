...
const DataExplorer = () => {
  ...
  if (tablesLoading) {
    return (
      <div className="py-20 text-center text-[13px] text-g-500">
        Loading tables...
      </div>
    );
  }
  ...
  return (
    <div className="flex gap-4" style={{ minHeight: '500px' }}>
      ...
      <div className="w-[240px] shrink-0">
        <div className="text-[14px] font-medium text-g-900 mb-3">
          Source tables
        </div>

        {/* Tables */}
        <div className="mb-4">
          <div className="text-[11px] text-g-400 uppercase tracking-wider mb-2">
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-[16px] font-medium text-g-900 font-mono">
                    {selectedTable}
                  </span>
                  <span
                ...
                {schema && (
                  <div className="text-[12px] text-g-400 mt-1">
...
const TableListItem = ({ table, selected, onClick }) => (
  ...
  >
    <div className="flex items-center justify-between">
      ...
      <span className="text-[11px] text-g-500 shrink-0 ml-2">
...
const SchemaView = ({ schema, loading }) => {
  if (loading || !schema) {
    return <div className="py-8 text-center text-[13px] text-g-500">Loading schema...</div>;
  }
      ...
      <table className="w-full text-[13px]">
        <thead>
          <tr className="bg-surface-card">
            <th className="text-left px-3 py-2 text-g-400 font-medium">#</th>
            <th className="text-left px-3 py-2 text-g-400 font-medium">Column</th>
            <th className="text-left px-3 py-2 text-g-400 font-medium">Type</th>
            <th className="text-left px-3 py-2 text-g-400 font-medium">Nullable</th>
            <th className="text-left px-3 py-2 text-g-400 font-medium">Default</th>
          </tr>
        </thead>
        <tbody>
          {schema.columns.map((col, i) => (
            ...
            >
              <td className="px-3 py-2 text-g-500">{col.position}</td>
              <td className="px-3 py-2 text-g-900 font-mono">{col.name}</td>
              ...
              <td className="px-3 py-2 text-g-400">
                {col.nullable ? 'yes' : 'no'}
              </td>
              <td className="px-3 py-2 text-g-500 font-mono text-[12px]">
                {col.default || '\u2014'}
              </td>
            </tr>
          ))}
...
const SampleDataView = ({ sample, loading }) => {
  if (loading || !sample) {
    return <div className="py-8 text-center text-[13px] text-g-500">Loading data...</div>;
  }

  if (sample.rows.length === 0) {
    return <div className="py-8 text-center text-[13px] text-g-500">No rows</div>;
  }
        ...
        <thead>
          <tr className="bg-surface-card">
            {sample.columns.map((col) => (
              <th
                key={col}
                className="text-left px-3 py-2 text-g-400 font-medium whitespace-nowrap"
              >
                {col}
              </th>
            ))}
          ...
          {sample.rows.map((row, i) => (
            ...
            >
              {sample.columns.map((col) => (
                <td
                  key={col}
                  className="px-3 py-1.5 text-g-700 whitespace-nowrap max-w-[200px] truncate"
                  title={row[col] != null ? String(row[col]) : ''}
                >
                  {row[col] != null ? String(row[col]) : (
                    <span className="text-g-400 italic">null</span>
                  )}
                </td>
              ))}
            </tr>
